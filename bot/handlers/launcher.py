...
async def menu_launch(call: CallbackQuery, device_id: str, lang: str):
    ...
    if not rows:
        await call.message.edit_text("No apps found.", reply_markup=back_kb(lang))
        return
    ...
    await call.message.edit_text("🚀 App Launcher:", reply_markup=apps_kb(rows, status_map, lang))
...
async def do_launch(call: CallbackQuery, device_id: str, lang: str):
    ...
    if rows:
        ...
        if result == "success":
             await call.message.edit_text(f"✅ {app_name} launched", reply_markup=back_kb(lang))
        else:
